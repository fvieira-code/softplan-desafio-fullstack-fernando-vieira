...
public class Secretaria extends AbstractEntity {
    ...
    @Column(name = "codigosec", nullable = false, unique = true)
    private String id;

    @Size(max = 100, message = "entity.situacao.descricao.size.message")
    @Column(name = "descricaosec", nullable = false, length = 100)
    private String descricaoSecretaria;

    @Enumerated(EnumType.STRING)
    @Column(name = "statussec", length = 20, insertable = false, updatable = false)
    private DominioStatusObjeto statusSecretaria;
